...
QList<Track> DataBase::getRandom()
{
    Log::db(Q_FUNC_INFO);
...
QList<Track> DataBase::getFavorite()
{
    if(!db.isOpen())
        db.open();
...
void DataBase::setArchive(QString ramoc_ID)
{
  QSqlQuery query(db);
...
void DataBase::unsetArchive(QString ramoc_ID)
{
  QSqlQuery query(db);
  QStringList fileList;
  QString stringExec;
...
void DataBase::setFavorite(QString ramoc_ID)
{
  QSqlQuery query(db);
  QString pfad;
  QString wertung;
...
void DataBase::setPlayed(Track track)
{
...
QList<Movie> DataBase::getAllTVShow()
{
  ...
  QList<QVariant> result;
  QList<Movie> returnList;
  reply = networkAccessManager->get(QNetworkRequest(QUrl("http://localhost/getAllTVShow.php")));
  connect(reply, SIGNAL(finished()), &loop, SLOT(quit()));
  loop.exec();
...
void DataBase::check_HDD(QString string_DirPath, QStringList* stringList)
{
      ...
      if(!dir.exists())
      {
        Log::error("DataBase::check_HDD  Verzeichnis existiert nicht");
        return;
      }
...
void DataBase::deleteMovie(QString movieID)
{
  QNetworkReply* reply;
  QHttpMultiPart *multiPart_Movie = new QHttpMultiPart(QHttpMultiPart::FormDataType);
  QEventLoop loop;
   ...
   reply= networkAccessManager->post(QNetworkRequest(QUrl("http://localhost/deleteFilm.php")), multiPart_Movie);
   multiPart_Movie->setParent(reply); // delete the multiPart with the reply
   connect(reply, SIGNAL(finished()), &loop, SLOT(quit()));
   loop.exec();
   reply->deleteLater();
...
QStringList DataBase::getArtistAlben(QString str)
{
  QStringList returnList;
...
QList<Track> DataBase::getTracks_Artist_Album(QString artist, QString album)
{

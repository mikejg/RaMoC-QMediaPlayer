...
//  {
    ...
    sig_NewInfo("Suche nach " + fileName);
...
void InsertMovies::insertTVShow()
{
...
void InsertMovies::insertMP3Files()
{
//  Log::error(Q_FUNC_INFO);
  ...
  while(strList_MP3Files->count() > 0)
  {
    ...
    if(!f.isNull())
    {
      ...
      sig_NewInfo("Schreibe " + mp.artist + "|" + mp.album + "|" + mp.title + " in Datenbank");
      database->insertMetaPaket(mp);
    }
    else
    {
      qDebug() << QDateTime::currentDateTime().toString("hh:mm:ss") << "Kein ID3Tag gefunden";
    }
    strList_MP3Files->pop_front();
  }
 sig_NewInfo(" ");
}
...
void InsertMovies::eval_TVQuery()
{
    ...
    {
      QVariantMap map = qVariant_Film.toMap();
      //qDebug() << QDateTime::currentDateTime().toString("hh:mm:ss") << map["original_name"].toString();
      if(map["original_name"].toString() == fileName)
      {
        movie.title = map["original_name"].toString();
        movie.tmdb_ID = map["id"].toString();
//        qDebug() << QDateTime::currentDateTime().toString("hh:mm:ss") << "Titel       : " << movie.title;
//        qDebug() << QDateTime::currentDateTime().toString("hh:mm:ss") << "TMDB_ID     : " << movie.tmdb_ID;
//        qDebug() << QDateTime::currentDateTime().toString("hh:mm:ss") << "Year        : " << movie.year;
//
        if(load_Details())
          database->insertMovie(movie);
        break;
...
void InsertMovies::check_NewFiles()
{
    ...
    foreach(tmpString, *strList_Files)
    {
      ...
      {
        stringList_databaseFiles.removeAll(tmpString);
      }
    }
}
